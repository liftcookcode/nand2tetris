...
class Assembler(object):
    ...
    def asssemble(self, asm_filename):
        ...
        # Second pass to write .hack files
        parser.reset_file()
        self.ram_address = 16
        while parser.has_more_commands:
            parser.advance()
    ...
    def write_L(self, symbol):
        address = self.create_address(self.parser_instruction_num+1)
        self.symbol_table.add_entry(symbol, address)
...
class Parser(object):
    ...
    def advance(self):
        '''
        Parse current instruction and load next instrcution
        '''
...
class Code(object):
    def dest(self, mnemonic):
        '''
        Alt: Enumerate all possibilites and do dictionary. lookup
        Current implentation is more flexible, but slower (max 9 comparisons vs 1 hashing)
        '''
        bin = ['0', '0', '0']
        if mnemonic is None:
            return ''.join(bin)
...
class SymbolTable(object):
    def __init__(self):
        ...
        self.ram_position = 16 # 0-15 present values

    def get_address(self, symbol):
        return self.symobl_dict[symbol]
    ...
    def base_table(self): # 15 bit adddresses, 32K locations
        return {
            'SP' : '000000000000000',
            'LCL' : '000000000000001',
            'ARG' : '000000000000010',
...
if __name__ =='__main__':
    ...
    assembler.asssemble(asm_filename)