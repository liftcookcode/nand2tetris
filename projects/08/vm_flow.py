...
def func_label_asm(name):
    ...
    return 'f{name}'.format(name=name)

def label(name, function):
...
def if_goto(name, function=None):
    ''' Go to a label like @goto based on a condition pops the first element from the stack, if non-zero, jump'''
    label_name = label_asm(name, function)
    return ''' @SP
...
def call(func, argc):
    ...
    arg_dist = int(argc) + 5 # The distance between SP and the args
    ...
    # _repush_label assumes the SP points to the top of element and leaves it this way
...
def function(func, locals):
    ...
    # For every other local arg, repeat this piece
    command += '''M=0
            @SP
            AM = M+1;\n'''*(locals_c -1)
...
def return_():
    ''' Retrun from a function, retrieving the previous stack frame'''
