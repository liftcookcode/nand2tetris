...
class CompilationEngine:
    ...
    def compile_class_vars(self, jack_class):
        ...
        while token is not None and token.type == 'keyword' and token.value in ['static', 'field']:
            # Advance here, to avoid, to avoid eating the token in the conditiona above
    ...
    def compile_class_subroutines(self, jack_class):
        ...
        while token is not None and token.type == 'keyword' and token.value in ['constructor', 'function', 'method']:
            ...
            jack_subroutine = CompilationTypes.JackSubroutine(name, subroutine_type, return_type, jack_class)
    ...
    def compile_subroutine_vars(self, jack_subroutine):
        ...
        token = self.tokenizer.current_token()
        # Check that a variable decleration starts
    ...
    def compile_parameter_list(self, jack_subroutine):
        ...
        while still_vars:
            # param type
            token = self.tokenizer.advance() # Dont advance to avoid eating
            param_type = token.value
            # param name
            param_name = self.tokenizer.advance().value
    ...
    def compile_statement_do(self, jack_subroutine):
        ...
        self.vm_writer.write_pop('temp', 0) # Pop to avoid filling the stack with trash
    ...
    def compile_expression(self, jack_subroutine):
        ...
        # In case of a function call or variable name
            ...
            if token.value == '[': # Array
                self.tokenizer.advance() # [
                self.compile_expression(jack_subroutine)
                self.vm_writer.write_pop_symbol(token_var)
                self.vm_writer.write('add')
                # rebase 'that' tp point to var+index
                ...
                # Used to mark wheter to use the default call, a method one
                default_call = True
                arg_count = 0
