# Need: Parser, Code, Main
# Step 1 :
# Handle stack arithmetic commands -> nine arithmetic/logical commands, as well as VM command push x
# Step 2:
# Handling memory access commands -> push,pop & 8 memory segments 
...
class Parser(object):
    ...
    def command_type(self):
        return self.commnads.get(self.curr_instruction[0].lower())
...
class CodeWriter(object):
    ...
    def write_arithmetic(self, operation):
        ''' Apply operation to top on stack'''
    ...
    def resolve_address(self, segment, index):
        ...
        address = self.address.get(segment)
        if segment == 'constant':
            self.write('@' + str(index))
        elif segment == 'static':
            self.write('@' + self.curr_file + '.' + str(index))
        elif segment in ['pointer', 'temp']:
            self.write('@R' + str(address + int(index)))    # Address in an int
...
class Main(object):
    ...
    def translate(self, vm_file):
        ...
        while parser.has_more_commands:
            parser.advance()
            self.write('//' + ''.join(parser.curr_instruction))
